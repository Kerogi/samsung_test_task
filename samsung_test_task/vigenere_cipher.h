...
{
	std::transform(begin, end, dst, vigenere_cipher<english_case_less>(english_case_less(), key));
}

template<typename IterInT, typename IterOutT>
void decrypt_vigenere(std::string const& key, IterInT begin, IterInT end, IterOutT dst)
{
	std::transform(begin, end, dst, vigenere_cipher<english_case_less>(english_case_less(), key, vigenere_cipher<english_case_less>::Decrypt));
}